...
#define EXTENSION_ARCHIVO "csv"
#define RECUPERAR_ARCHIVO ".csv"

#define DESCRIPCION_SALIR  "Salir"
#define DESCRIPCION_VOLVER  "Volver atras"
...
void imprimir_espacio(interfaz_t* interfaz, int tipo_espacio, size_t espaciado){
    if(tipo_espacio == INICIO)
        imprimir_margen(interfaz->margen);
    determinar_color_fondo(interfaz->color_fondo);
...
void imprimir_enter(interfaz_t* interfaz){
    imprimir_margen(interfaz->margen);
    imprimir_espacio(interfaz, FIN, interfaz->max);
    determinar_color(RESET);
}
/* 
...
void imprimir_linea(interfaz_t* interfaz, size_t espaciado, const char* color, const char* linea){
    ...
    printf("%s", linea);
    determinar_color(RESET);
    size_t limite = interfaz->max - espaciado - strlen(linea);
...
void imprimir_linea_partida(interfaz_t* interfaz, const char* color_1, const char* linea_1, const char* color_2, const char* linea_2){
    imprimir_espacio(interfaz, INICIO, interfaz->espaciado);
    size_t limite;
    ...
    printf("%s", linea_2);
    determinar_color(RESET);
    limite = interfaz->max - interfaz->espaciado - strlen(linea_1) - strlen(linea_2);
    imprimir_espacio(interfaz, FIN, limite);
}
//interfaz.h
...
*/
void imprimir_opcion(interfaz_t* interfaz, char clave, const char* descripcion, char color[MAX_COLOR]){
    char linea_opcion[LONGITUD_OPCION];            
    char linea_descripcion[interfaz->max - LONGITUD_OPCION];
    sprintf(linea_opcion," >> ( %c )  ", clave);             
    sprintf(linea_descripcion,"-  %s", descripcion);             
    imprimir_linea_partida(interfaz, color, linea_opcion, NEGRO, linea_descripcion);   
}
/*
*
* Pre :
* Post:
*/
void mostrar_opcion(interfaz_t* interfaz, menu_t menu, size_t pos){
    imprimir_enter(interfaz);
    imprimir_opcion(interfaz, menu.opciones[pos], menu.descripciones[pos], menu.color_titulo);
    imprimir_enter(interfaz);
}
/* 
...
*/
void mostrar_opciones(interfaz_t* interfaz, size_t tipo_menu){
    menu_t menu = interfaz->menus[tipo_menu];

    if(tipo_menu == 0)
        for(size_t pos = 1; pos < menu.cant_opciones; pos++)
            mostrar_opcion(interfaz, menu, pos);
    else
        for(size_t pos = 2; pos < menu.cant_opciones; pos++)
            mostrar_opcion(interfaz, menu, pos);

    imprimir_barra(interfaz, INICIO, menu.color_titulo);
    if(tipo_menu != 0)
        mostrar_opcion(interfaz, menu, VOLVER);
    mostrar_opcion(interfaz, menu, SALIR);
...
* Post:
*/
void menu_encabezado(interfaz_t* interfaz, size_t menu){
    size_t espaciado = (interfaz->max - strlen(interfaz->menus[menu].titulo)) / 2;
    imprimir_marco(interfaz, INICIO);
    imprimir_enter(interfaz);
    imprimir_barra(interfaz, INICIO, interfaz->menus[menu].color_titulo);
    imprimir_enter(interfaz);
    imprimir_linea(interfaz, espaciado, interfaz->menus[menu].color_titulo, interfaz->menus[menu].titulo);
    imprimir_enter(interfaz);
    imprimir_barra(interfaz, FIN, interfaz->menus[menu].color_titulo);
    imprimir_enter(interfaz);
}
...
*/
bool ruta_archivo_valida(char* ruta){
...
interfaz_t* interfaz_crear(size_t max, size_t margen, size_t espaciado){
    ...
    interfaz->max = max;
    interfaz->margen = margen;
    interfaz->espaciado = espaciado;
    interfaz->menus = NULL;
    interfaz->cant_menus = 0;
    strcpy(interfaz->color_fondo, FONDO_BLANCO);
    return interfaz;
}
//interfaz.h
...
int menu_insertar(interfaz_t* interfaz, char titulo[MAX_DESCRIPCION], char color_titulo[MAX_COLOR]){
    ...
    strcpy(interfaz->menus[tope].color_titulo, color_titulo);
    interfaz->menus[tope].cant_opciones = 0;
    cargar_opcion(interfaz, tope, OPCION_SALIR, DESCRIPCION_SALIR);
    if(tope != 0)
        cargar_opcion(interfaz, tope, OPCION_VOLVER, DESCRIPCION_VOLVER);
    interfaz->cant_menus ++;
    return (int)interfaz->cant_menus;
}
//interfaz.h
void menu_mostrar(interfaz_t* interfaz, size_t menu){
    if(!interfaz || menu >= interfaz->cant_menus){
        warning("la interfaz o menú utilizado");
        return;
    }
    system(LIMPIAR);
    menu_encabezado(interfaz, menu);
    mostrar_opciones(interfaz, menu);
}
//interfaz.h
...
size_t pedir_numero(interfaz_t* interfaz){
    imprimir_margen(interfaz->margen);
    printf("Ingrese el" VERDE "número" RESET "que desea : ");
    size_t numero;
    scanf(" %li", &numero);
    return numero;
}
...
char* pedir_archivo_csv(interfaz_t* interfaz){
    char* ruta_archivo;
    imprimir_marge(interfaz->margen);
    ...
    while(!ruta_archivo_valida(ruta_archivo)){
        ...
        imprimir_espaciado(interfaz->margen);
